...
from test.modules.hrtime_module import HrTimeModule
...
class TestHrTimeModule(unittest.TestCase):
    ...
    def test_lsmod(self):
        ...
        '''
        with HrTimeModule(path=MODULE_PATH) as time_module:
    ...
    def test_init_message(self):
        ...
        Dmesg.clear()
        with HrTimeModule(path=MODULE_PATH) as time_module:
            _, func, msg = next(Dmesg.get_messages(HrTimeModule.MODULE_NAME, last=1))

            self.assertEqual("ex_hrtimer_init", func.strip())
    ...
    def test_exit_message(self):
        ...
        self.assertEqual("ex_hrtimer_exit", func.strip())
    ...
    def test_permission(self):
        ...
        with HrTimeModule(path=MODULE_PATH) as time_module:
            param = {
                time_module._get_parameter_path(HrTimeModule.MODULE_PARAM_CMD): 0o640,
                time_module._get_parameter_path(HrTimeModule.MODULE_PARAM_CMD_STATUS): 0o640,
                time_module._get_parameter_path(HrTimeModule.MODULE_PARAM_CMD_RESULT): 0o640,
            }
    ...
    def test_unknown_cmd_start_with_space(self):
        ...
        with HrTimeModule(path=MODULE_PATH) as time_module:
            with self.assertRaises(OSError):
                 status, result = time_module.execute(" "+HrTimeModule.MODULE_CMD_RUN)
    ...
    def test_set_period(self):
        ...
        '''
        with HrTimeModule(path=MODULE_PATH) as time_module:
            status, result = time_module.execute(f'{HrTimeModule.MODULE_CMD_PERIOD} 5')
    ...
    def test_set_duty(self):
        ...
        '''
        with HrTimeModule(path=MODULE_PATH) as time_module:
            status, result = time_module.execute(f'{HrTimeModule.MODULE_CMD_DUTY} 1')
            self.assertEqual(0, status)
    ...
    def test_run_5_min_duty(self):
        ...
        with HrTimeModule(path=MODULE_PATH) as time_module:
              ...
              for i in range(default_duty):
                   time.sleep(62)
              status, result = time_module.execute(HrTimeModule.MODULE_CMD_STOP)
              self.assertEqual(0, status)
              self.assertEqual(0, result)

        ms_gen = Dmesg.get_reg_messages_by_func(HrTimeModule.MODULE_NAME, "pulse_callback", "min=\\d{1,3}:\\sHello, timer!")
    ...
    def test_stop_before_run(self):
        ...
        ms_gen = Dmesg.get_messages(HrTimeModule.MODULE_NAME, last=3)
        _, func, msg = next(ms_gen)
        self.assertEqual("ex_hrtimer_init", func.strip())
        self.assertEqual("init", msg.strip())
        _, func, msg = next(ms_gen)
