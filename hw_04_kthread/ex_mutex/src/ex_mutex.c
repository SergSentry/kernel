...
static int writer_thread(void *arg) {
  while (!kthread_should_stop()) {
    if (!mutex_trylock(&sync_mutex)) {
      pr_warn("Couldn't acquire mutex immediately.\n");
      continue;
    }
...
static int __init ex_mutex_init(void) {
  if (!mutex_is_locked(&sync_mutex)) {
    mutex_lock(&sync_mutex);
    mutex_unlock(&sync_mutex);
  } else {
    pr_err("Failed to initialize mutex.\n");
    return -EFAULT;
  }
