...
class RbTreeModule:
    MODULE_FILE_NAME = 'ex_rb_tree.ko'
    ...
    def has_loaded(self) -> bool:
        result = subprocess.run(
            [f"lsmod | awk '/^{RbTreeModule.MODULE_NAME}[ ]+[0-9]+[ ]+[0-9]+/ {{print $0}}' | tr -s ' '"],
            stdout=subprocess.PIPE, shell=True)
        if result.stderr is not None:
            return False
        params = result.stdout.decode("utf-8").split(" ")
        return RbTreeModule.MODULE_NAME in params

    def execute(self, cmd: str):
        self._write_param(RbTreeModule.MODULE_PARAM_CMD, cmd)
        status = self._read_param(RbTreeModule.MODULE_PARAM_CMD_STATUS)
        result = self._read_param(RbTreeModule.MODULE_PARAM_CMD_RESULT)
        return (status, result)

    def _get_parameter_path(self, name):
        return f'/sys/module/{RbTreeModule.MODULE_NAME}/parameters/{name}'

    def __unload(self):
        if self.has_loaded():
            result = subprocess.run([f'rmmod {RbTreeModule.MODULE_NAME}'], stdout=subprocess.PIPE, shell=True)
            if result.stderr is not None:
                return False
            response = result.stdout.decode("utf-8").split(" ")
            return RbTreeModule.MODULE_NAME in response

        return True

    def __load_default(self, path: str) -> bool:
        self.__unload()
        module_full_path = os.path.abspath(f"{path}/{RbTreeModule.MODULE_FILE_NAME}")
    ...
    def _write_param(self, param_name: str, value):
        if param_name not in {RbTreeModule.MODULE_PARAM_CMD, RbTreeModule.MODULE_PARAM_CMD_STATUS,
                              RbTreeModule.MODULE_PARAM_CMD_RESULT}:
