...
class HashModule:
    ...
    def has_loaded(self) -> bool:
        result = subprocess.run(
            [f"lsmod | awk '/^{HashModule.MODULE_NAME}[ ]+[0-9]+[ ]+[0-9]+/ {{print $0}}' | tr -s ' '"],
            stdout=subprocess.PIPE, shell=True)
        if result.stderr is not None:
            return False
        params = result.stdout.decode("utf-8").split(" ")
        return HashModule.MODULE_NAME in params

    def execute(self, cmd: str):
        self._write_param(HashModule.MODULE_PARAM_CMD, cmd)
        status = self._read_param(HashModule.MODULE_PARAM_CMD_STATUS)
        result = self._read_param(HashModule.MODULE_PARAM_CMD_RESULT)
        return (status, result)

    def _get_parameter_path(self, name):
        return f'/sys/module/{HashModule.MODULE_NAME}/parameters/{name}'

    def __unload(self):
        if self.has_loaded():
            result = subprocess.run([f'rmmod {HashModule.MODULE_NAME}'], stdout=subprocess.PIPE, shell=True)
            if result.stderr is not None:
                return False
            response = result.stdout.decode("utf-8").split(" ")
            return HashModule.MODULE_NAME in response

        return True

    def __load_default(self, path: str) -> bool:
        self.__unload()
        module_full_path = os.path.abspath(f"{path}/{HashModule.MODULE_FILE_NAME}")
    ...
    def _write_param(self, param_name: str, value):
        if param_name not in {HashModule.MODULE_PARAM_CMD, HashModule.MODULE_PARAM_CMD_STATUS,
                              HashModule.MODULE_PARAM_CMD_RESULT}:
