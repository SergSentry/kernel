...
void my_callback_function(struct tasklet_struct *work) {
  write_lock(&rwlock);
  global_value++;
...
DECLARE_TASKLET(my_work, my_callback_function);
...
static int __init ex_tasklet_init(void) {
  pr_info("init\n");
  return 0;
}

static void __exit ex_tasklet_exit(void) {
  if (work_task != NULL) {
    kthread_stop(work_task);
    work_task = NULL;
  }

  tasklet_kill(&my_work);
...
module_init(ex_tasklet_init);
module_exit(ex_tasklet_exit);
