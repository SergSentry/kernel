...
static int get_mem_cmd_handler(const char *argv) {
  ...
  while (max_size > 0) {
    pool = mempool_create(1, mempool_kmalloc, mempool_kfree, (void *)max_size);
    if (pool) {
      start_time = ktime_get_ns();
      test_ptr = mempool_alloc(pool, GFP_KERNEL);
  ...
  if (!test_ptr) {
    ...
    pr_err("mempool: FAIL, err_msg = no memory allocated!\n");
  ...
  char *type = check_physical_continuity(test_ptr, max_size) ? "CONTINUOUS"
                                                             : "NOT CONTINUOUS";
  pr_info("mempool: %zu byte, %zu ms, type: %s\n", max_size,
          time_mem_allocating, type);

  mempool_free(test_ptr, pool);
  test_ptr = NULL;
...
static int get_page_mem_cmd_handler(const char *argv) {
  ...
  while (max_size > 0) {
    pool = mempool_create(1, mempool_alloc_pages, mempool_free_pages,
                          (void *)(long)max_size);
    if (pool) {
      start_time = ktime_get_ns();
      test_ptr = mempool_alloc(pool, GFP_KERNEL);
