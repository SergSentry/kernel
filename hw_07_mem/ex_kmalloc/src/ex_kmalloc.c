...
static int get_mem_cmd_handler(const char *argv) {
  ...
  while (max_size > 0) {
    ...
    test_ptr = kmalloc(max_size, GFP_KERNEL);
  ...
  if (!test_ptr) {
    pr_err("kmalloc: FAIL, err_msg = no memory allocated!\n");
    return STATUS_CMD_ERROR_MEMORY;
  }

  unsigned long time_mem_allocating = (end_time - start_time);

  pr_info("kmalloc: SUCCSESS\n");

  char *type = check_physical_continuity(test_ptr, max_size) ? "CONTINUOUS"
                                                             : "NOT CONTINUOUS";
  pr_info("kmalloc: %zu byte, %zu ms, type: %s\n", max_size,
